...
function ElasticSearchBackend(params) {
  ...
  var elasticVersion = this.client.transport._config.apiVersion;
  this.useOldFilterQuery = (elasticVersion === '1.7' || elasticVersion === '0.9');
...
ElasticSearchBackend.prototype = {
  ...
  _search: function (bucket, keys, cb, size) {
    ...
    var query = makeQuery('key', keys, _this.useOldFilterQuery);
  ...
  del: function (transaction, bucket, keys) {
    ...
    transaction.push(function (cb) {
      ...
      _this.client.deleteByQuery({
        ...
        body: makeQuery('key', keys, _this.useOldFilterQuery),
        ignore: [404]
      }, function (err) {
        if (err instanceof Error) return cb(err);
        cb(undefined);
...
function makeQuery(field, arr, useOldFilter) {
  if (useOldFilter) {
    return makeQueryOld(field, arr);
  }
  else {
    return makeQueryNew(field, arr);
  }
}


function makeQueryNew(field, arr) {
  arr = makeArray(arr);
  var body = {
    "query": {
      "bool" :{
...
function makeQueryOld(field, arr) {
  arr = makeArray(arr);
  var body = {
    "query": {
      "filtered": {
